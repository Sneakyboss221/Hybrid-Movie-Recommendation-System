...
class HybridRecommender:
    ...
    def explain_recommendation(self, movie_title: str, recommended_movie_title: str, 
        ...
        content_explanation = self.content_recommender.explain_recommendation(movie_title, recommended_movie_title)
        if content_explanation:
            explanations.append(f"Content-based: {content_explanation}")
        
        # Collaborative explanation (if user_id provided)
        if user_id:
            ...
            if user_profile and 'favorite_genres' in user_profile:
                favorite_genres = [genre for genre, _ in user_profile['favorite_genres'][:3]]
                
                # Check if recommended movie matches user's favorite genres
                recommended_movie_info = self.movies_df[self.movies_df['clean_title'] == recommended_movie_title]
                if not recommended_movie_info.empty:
                    movie_genres = recommended_movie_info.iloc[0]['genres'].split('|')
                    matching_genres = [genre for genre in movie_genres if genre in favorite_genres]
                    
                    if matching_genres:
                        explanations.append(f"Collaborative: You enjoy {', '.join(matching_genres)} movies, and this movie matches your preferences.")
        ...
        recommended_movie_id = self.title_to_movie_id.get(recommended_movie_title)
        if recommended_movie_id:
            movie_stat = movie_stats[movie_stats['movieId'] == recommended_movie_id]
            if not movie_stat.empty:
                rating_count = movie_stat.iloc[0]['rating_count']
                rating_mean = movie_stat.iloc[0]['rating_mean']
                explanations.append(f"Popularity: This movie has been rated {rating_count} times with an average rating of {rating_mean:.1f}/5.")
        
        if explanations:
            return " | ".join(explanations)
        else:
            return "No specific explanation available for this recommendation."
